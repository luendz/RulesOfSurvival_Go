...
namespace ROS.Game.Animation
{
    ...
    public sealed class PlayerAnimationCoordinator : MonoBehaviour
    {
        ...
        private const string GestureIdleState = "Empty";
        ...
        private bool _wasSwitchingWeapon;
        private bool _vehicleActive;
        private int _vehicleRole;
        private int _vehicleStyle;
        private float _vehicleSpeed;
        private float _vehicleSteer;
        ...
        private static readonly int WeaponSwitch = Animator.StringToHash("WeaponSwitch");
        ...
        private static readonly int ParachuteState = Animator.StringToHash("ParachuteState");
        ...
        private static readonly int ClassicLean = Animator.StringToHash("Lean");
        ...
        public void SetVehicleAnimationState(
            bool active,
            int role,
            int style,
        ...
        )
        {
            ...
            _vehicleRole = active ? role : 0;
            _vehicleStyle = active ? style : 0;
            _vehicleSpeed = active ? Mathf.Clamp01(normalizedSpeed) : 0f;
            _vehicleSteer = active ? Mathf.Clamp(steer, -1f, 1f) : 0f;
        }

        public bool CanPlayGestureState(bool fullBody, string stateName)
        {
            return TryResolveGestureState(
                fullBody,
                stateName,
                out _,
                out _,
        ...
        public bool TryPlayGestureState(
            bool fullBody,
            string stateName,
        ...
        public bool IsGestureLayerIdle(int layerIndex)
        {
            if (animator == null ||
                layerIndex < 0 ||
            ...
            return state.shortNameHash == Animator.StringToHash(GestureIdleState);
        }

        public void StopGestureState(
            int layerIndex,
            string layerName,
            float transitionDuration
        )
        {
            ...
            int idleHash = Animator.StringToHash(
                $"{layerName}.{GestureIdleState}"
            );
            if (animator.HasState(layerIndex, idleHash))
            {
                animator.CrossFade(
                    idleHash,
                    Mathf.Max(0f, transitionDuration),
        ...
        )
        {
            ...
            layerIndex = animator.GetLayerIndex(primary);
            layerName = primary;
            if (layerIndex < 0)
            {
                layerIndex = animator.GetLayerIndex(fallback);
                layerName = fallback;
            }
        ...
        private void UpdateBaseStateParameters()
        {
            ...
            bool grounded = _vehicleActive || (!airborneDrop && motor.IsGrounded);
            bool dead = health != null && !health.IsAlive;
            ...
            SetIntegerIfPresent(
                ParachuteState,
                ResolveParachuteAnimationState(
                    parachute != null ? parachute.State : AirDropState.Inactive
                )
            );
            SetBoolIfPresent(
        ...
        private static int ResolveParachuteAnimationState(AirDropState state)
        {
            switch (state)
            {
        ...
        private void UpdateUpperBodyParametersAndLayers()
        {
            ...
            bool vehicleFullBody = !dead && _vehicleActive;
            ...
            if (switchingWeapon && !_wasSwitchingWeapon)
                SetTriggerIfPresent(WeaponSwitch);
            _wasSwitchingWeapon = switchingWeapon;
            ...
            SetBoolIfPresent(LegacyHasRifle, firearm);
            SetBoolIfPresent(LegacyAim, classicAiming);
            SetFloatIfPresent(
                ClassicLean,
                leanController != null ? leanController.CurrentLean : 0f
            );
            ...
            SetIntegerIfPresent(VehicleState, _vehicleActive ? 1 : 0);
            SetIntegerIfPresent(VehicleRole, _vehicleActive ? _vehicleRole : 0);
            SetIntegerIfPresent(VehicleStyle, _vehicleActive ? _vehicleStyle : 0);
            SetFloatIfPresent(VehicleSpeed, _vehicleActive ? _vehicleSpeed : 0f);
            SetFloatIfPresent(VehicleSteer, _vehicleActive ? _vehicleSteer : 0f);
