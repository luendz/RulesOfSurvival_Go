...
namespace ROS.Game.Loot
{
    ...
    public sealed class PlayerLootEquipment : MonoBehaviour
    {
        ...
        )
        {
            ...
            {
                Debug.LogError($"'{item.displayName}' no tiene weaponPrefab configurado.", this);
                return null;
            }

            GameObject weaponObject = Instantiate(item.weaponPrefab, transform);
            ...
            {
                Debug.LogError(
                    $"El prefab de '{item.displayName}' no contiene {nameof(WeaponController)}.",
                    weaponObject);
