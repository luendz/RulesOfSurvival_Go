...
{
    // Runs once per Unity session (and after recompile).
    // Also available manually via ROS > Audio > Setup Audio Clips.
    [InitializeOnLoad]
    public static class AudioSetupWizard
    {
        private const string SessionKey = "ROS.AudioSetupWizard.Done";
        private const string AudioRoot   = "Assets/_Game/Audio";

        static AudioSetupWizard()
        {
            if (SessionState.GetBool(SessionKey, false))
                return;

            // Delay so all assets finish importing before we touch prefabs.
